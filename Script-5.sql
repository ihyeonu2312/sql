...
-- 학생 테이블에서 이름순으로 정렬 (기본값 오름차순 ASC) / 이름, 학년, 전화번호 조회
...
-- 학생 테이블에서 학년을 내림차순으로 정렬, 같은 학년일 경우 이름순으로 정렬
-- / 이름, 학년, 전화번호 조회
SELECT NAME, GRADE, TEL
FROM STUDENT s 
ORDER BY GRADE DESC, NAME;

-- 교수 번호으로 기준 내림차순 / 학번, 이름 조회
SELECT STUDNO, NAME
FROM STUDENT s 
...
-- 함수
-- 학생의 이름, 아이디 조회 / INITCAP 첫글자를 대문자로 / UPPER 모든 글자를 대문자로 / LOWER 모든 글자를 소문자로

SELECT NAME, USERID, INITCAP(USERID), UPPER(USERID), LOWER(USERID) 
FROM STUDENT;

-- 부서의 이름을 조회하고 이름의 길이와, 바이트 갯수를 조회 // LENGTHB 문자의 바이트 수
...
SELECT TEL, RPAD(SUBSTR(TEL, 1, INSTR(TEL,'-')),LENGTH(TEL),'*' ) 
FROM STUDENT;
...
-- 부서 테이브렝서 부서 이름의 마지막 '과' 글자를 제거
SELECT DNAME, RTRIM(DNAME, '과') 
FROM DEPARTMENT;

-- 교수 테이블에서 일급 계산(1달 22일 가정)
-- 일급 각각 소수점 첫째자리에서, 소수점 셋째 자리에서 반올림, 십의 자리로 반올림
-- 일급 각각 소수점 첫째자리에서, 소수점 셋째 자리에서 절삭
...
-- 교수번호 9908교수님의 입사일, 입사 30일 후, 입사 60일 후의 날짜를 조회
...
SELECT ROUND(MONTHS_BETWEEN(SYSDATE, '96/12/23')) A FROM DUAL; 

-- 입사한지 320개월 된 교수들을 조회 / 교수번호, 입사일, 입사일 +6개월 후, 입사일로 부터 현재까지의 개월 수 계산
SELECT 
	PROFNO,
	HIREDATE,
...
SELECT SYSDATE, LAST_DAY(SYSDATE), NEXT_DAY(SYSDATE, 7) 
FROM DUAL;

-- 오늘을 조회, 반올림, 절삭처리
SELECT SYSDATE-4/24, ROUND(SYSDATE-4/24), TRUNC(SYSDATE-4/24) 
FROM DUAL;

-- 오늘을 반올림, 날짜, 월, 연으로 반올림
SELECT 
	SYSDATE,
